...
import 'package:otzaria/seforim/migration/models/line.dart' as migration_models;
...
void main() {
  ...
  group('GimatriaSearch — חיפוש על פני שני DBs', () {
    ...
    test('כשספר נמצא רק ב-seforim — תוצאות גימטריה נמשכות משם', () async {
      ...
      expect(results.any((r) => r.file == 'בראשית' && r.text.contains('אב')),
          isTrue);
    });
    ...
    test(
      ...
      expect(files, containsAll(['ספר רשמי', 'ספר אישי']),
