...
class EmptyLibraryBloc extends Bloc<EmptyLibraryEvent, EmptyLibraryState> {
  ...
  Future<void> _onImportExistingLibraryRequested(
    ...
    try {
      await _importExisting(
          event.sourcePath, event.targetPath, event.isArchive, emit);
    } catch (e) {
      emit(_error(
  ...
  Future<void> _importLibraryFolder(
      String source, String target, Emitter<EmptyLibraryState> emit) async {
    ...
    // אל ספרייה קיימת).
    final dbZst =
        File(path.join(source, DatabaseConstants.databaseArchiveFileName));
    final dbPlain = File(path.join(source, DatabaseConstants.databaseFileName));
    final targetDb =
        File(path.join(target, DatabaseConstants.databaseFileName));
    ...
    final lexical =
        File(path.join(source, DatabaseConstants.lexicalDatabaseFileName));
    if (await lexical.exists()) {
      await lexical
          .copy(path.join(target, DatabaseConstants.lexicalDatabaseFileName));
    }

    // תלמוד בבלי — ארכיון tar.zst או תיקייה מחולצת, אופציונלי
    final talmudArchive =
        File(path.join(source, DatabaseConstants.talmudBavliArchiveFileName));
    final talmudDir =
        Directory(path.join(source, DatabaseConstants.talmudBavliFolderName));
    if (await talmudArchive.exists()) {
      await _extractTarArchive(talmudArchive.path, target,
          _extractProgress(emit, source, 'מחלץ ספרי תלמוד בבלי...'));
    } else if (await talmudDir.exists()) {
      await copyDirectoryEntries(talmudDir.path,
  ...
  Future<void> _importExisting(String sourcePath, String targetPath,
      bool isArchive, Emitter<EmptyLibraryState> emit) async {
  ...
  Future<void> _onUpdateLibraryRequested(
      UpdateLibraryRequested event, Emitter<EmptyLibraryState> emit) async {
      ...
      for (final suffix in _dbFileSuffixes) {
        ...
        if (await f.exists()) {
          await _moveFile(
              f,
              path.join(
                  backup.path, '${DatabaseConstants.databaseFileName}$suffix'));
        }
      }
    } catch (_) {
  ...
  Future<void> _copyDatabaseFiles(String sourceDir, String targetDir) async {
    final sourceDb =
        File(path.join(sourceDir, DatabaseConstants.databaseFileName));
  ...
  Future<void> _handleZstFile(
      String zstFilePath, Emitter<EmptyLibraryState> emit,
    ...
    try {
      ...
      if (basename == DatabaseConstants.databaseArchiveFileName.toLowerCase()) {
        await _extractBundleSiblings(
            path.dirname(zstFilePath), targetDir, emit);
      }
    ...
    if (await talmudArchive.exists()) {
      try {
        ...
        await _extractTarArchive(
          talmudArchive.path,
          outputDir,
          _extractProgress(emit, talmudArchive.path, 'מחלץ ספרי תלמוד בבלי...'),
        );
      } catch (e) {
  ...
  Future<void> _handleZipFile(
      String zipFilePath, Emitter<EmptyLibraryState> emit,
      {String? outputDir}) async {
  ...
  Future<void> _onDownloadLibraryRequested(
    ...
    try {
      final libraryPath = event.targetPath ??
          _defaultLibraryPathOverride ??
          await AppPaths.getDefaultLibraryPath();
  ...
  Future<void> _downloadLibrary(
      String libraryPath, Emitter<EmptyLibraryState> emit) async {
    {
